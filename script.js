// =============================================================================
// ARQUIVO: script.js
// SISTEMA LOGIMASTER - VERSÃO FINAL 3.5 (INTEGRAL E ROBUSTA)
// =============================================================================
...
// VARIÁVEIS DO SISTEMA DE CRÉDITOS
...
// Remove acentos e converte para minúsculo (Útil para busca e categorização)
    ...
    async function baixarColecao(chave, setter) {
        try {
            ...
            if (docSnap.exists()) {
                ...
                // Define se é objeto único ou lista
...
// Buscas Rápidas (Helpers de acesso)
        ...
        try {
            ...
            if (criarLogin) {
                ...
                await window.dbRef.setDoc(window.dbRef.doc(window.dbRef.db, "users", novoUID), {
                    ...
                    approved: true, senhaVisual: senha
                });
            }

            var funcionarioObj = {
                id: novoUID, nome: nome, funcao: funcao, 
                documento: document.getElementById('funcDocumento').value,
                email: email, telefone: document.getElementById('funcTelefone').value, 
            ...
            console.error(erro); alert("Erro: " + erro.message); 
        } finally { 
            btnSubmit.disabled = false; btnSubmit.innerHTML = textoOriginal; 
        }
    }
});

// LISTENERS DEMAIS FORMULÁRIOS
...
document.addEventListener('submit', function(e) { 
    if (e.target.id === 'formContratante') { 
        ...
        var novo = { cnpj: cnpj, razaoSocial: document.getElementById('contratanteRazaoSocial').value.toUpperCase(), telefone: document.getElementById('contratanteTelefone').value }; 
...
document.addEventListener('submit', function(e) { 
    if (e.target.id === 'formMinhaEmpresa') { 
        e.preventDefault(); 
        var dados = { razaoSocial: document.getElementById('minhaEmpresaRazaoSocial').value.toUpperCase(), cnpj: document.getElementById('minhaEmpresaCNPJ').value, telefone: document.getElementById('minhaEmpresaTelefone').value }; 
...
document.addEventListener('submit', function(e) {
    if (e.target.id === 'formDespesaGeral') {
        ...
        var novaDespesa = {
            id: id, data: document.getElementById('despesaGeralData').value,
        ...
        salvarListaDespesas(lista).then(() => {
            alert("Despesa Lançada!"); e.target.reset(); document.getElementById('despesaGeralId').value = '';
            toggleDespesaParcelas(); renderizarTabelaDespesasGerais(); atualizarDashboard(); 
        });
    }
});
...
document.addEventListener('submit', function(e) {
    if (e.target.id === 'formOperacao') {
        ...
        if (opAntiga && !isAgendamento) {
            if (opAntiga.status === 'EM_ANDAMENTO' || opAntiga.status === 'FINALIZADA') statusFinal = opAntiga.status; 
        }
        ...
        var novaOp = {
            ...
            status: statusFinal, checkins: checkinsData, ajudantes: window._operacaoAjudantesTempList || [],
            kmInicial: opAntiga ? opAntiga.kmInicial : 0, kmFinal: opAntiga ? opAntiga.kmFinal : 0
        };
        ...
        salvarListaOperacoes(lista).then(() => {
            alert(isAgendamento ? "Operação Agendada!" : "Operação Salva!");
...
window.limparOutroFiltro = function(tipo) { if (tipo === 'motorista') { document.getElementById('filtroMotoristaGrafico').value = ""; } else { document.getElementById('filtroVeiculoGrafico').value = ""; } };

// FUNÇÃO MESTRE DE ATUALIZAÇÃO VISUAL
...
function renderizarTabelaDespesasGerais() {
    var tbody = document.querySelector('#tabelaDespesasGerais tbody'); if (!tbody) return; tbody.innerHTML = '';
    CACHE_DESPESAS.sort((a,b) => new Date(b.data) - new Date(a.data)).forEach(d => {
        ...
        tbody.innerHTML += `<tr><td>${formatarDataParaBrasileiro(d.data)}</td><td>${d.veiculoPlaca || 'GERAL'}</td><td>${d.descricao}</td><td style="color:var(--danger-color); font-weight:bold;">${formatarValorMoeda(d.valor)}</td><td>${textoPgto}</td><td><button class="btn-mini delete-btn" onclick="excluirDespesa('${d.id}')"><i class="fas fa-trash"></i></button></td></tr>`;
    });
}

window.excluirDespesa = function(id) { if(!confirm("Excluir?")) return; var lista = CACHE_DESPESAS.filter(d => String(d.id) !== String(id)); salvarListaDespesas(lista).then(() => { renderizarTabelaDespesasGerais(); atualizarDashboard(); }); };

function renderizarTabelaFuncionarios() { 
    var tbody = document.querySelector('#tabelaFuncionarios tbody'); if (!tbody) return; tbody.innerHTML = ''; 
    CACHE_FUNCIONARIOS.forEach(f => { tbody.innerHTML += `<tr><td>${f.nome}</td><td>${f.funcao}</td><td>${f.email||'-'}</td><td><button class="btn-mini edit-btn" onclick="preencherFormularioFuncionario('${f.id}')"><i class="fas fa-edit"></i></button> <button class="btn-mini delete-btn" onclick="excluirFuncionario('${f.id}')"><i class="fas fa-trash"></i></button></td></tr>`; }); 
}

window.excluirFuncionario = async function(id) { if(!confirm("Excluir?")) return; if (window.dbRef) try { await window.dbRef.deleteDoc(window.dbRef.doc(window.dbRef.db, "users", id)); } catch(e) {} var lista = CACHE_FUNCIONARIOS.filter(f => String(f.id) !== String(id)); await salvarListaFuncionarios(lista); alert("Removido."); preencherTodosSelects(); };
...
window.renderizarTabelaMonitoramento = function() {
    ...
    document.getElementById('badgeCheckins').style.display = pendentes.length > 0 ? 'inline-block' : 'none';
    if(pendentes.length > 0) document.getElementById('badgeCheckins').textContent = pendentes.length;

    pendentes.forEach(function(op) {
        var cliente = buscarContratantePorCnpj(op.contratanteCNPJ)?.razaoSocial || 'CLIENTE';
        var statusHtml = op.status === 'EM_ANDAMENTO' ? '<span class="status-pill" style="background:orange; animation:pulse 2s infinite">EM ROTA</span>' : '<span class="status-pill pill-pending">AGENDADA</span>';
        ...
        if (mot) {
            ...
            tbody.innerHTML += `<tr><td>${formatarDataParaBrasileiro(op.data)}</td><td><strong>${mot.nome}</strong><br><small>${op.veiculoPlaca}</small></td><td>${cliente}</td><td>${statusHtml}</td><td>${statusEq}</td><td>${btnFalta}</td></tr>`;
        }
        if (op.ajudantes) {
            op.ajudantes.forEach(ajItem => {
                var aj = buscarFuncionarioPorId(ajItem.id);
                if(aj) {
                    var faltouAj = (op.checkins?.faltas?.[aj.id]);
...
window.renderizarTabelaFaltas = function() {
    var tbody = document.querySelector('#tabelaFaltas tbody'); if(!tbody) return; tbody.innerHTML = '';
    CACHE_OPERACOES.forEach(function(op) {
        if (!op.checkins) return;
        if (op.checkins.faltaMotorista) { var m=buscarFuncionarioPorId(op.motoristaId); if(m) tbody.innerHTML += `<tr><td>${formatarDataParaBrasileiro(op.data)}</td><td style="color:red;">${m.nome}</td><td>MOTORISTA</td><td>FALTA</td><td>-</td></tr>`; }
        if (op.checkins.faltas) { Object.keys(op.checkins.faltas).forEach(k => { if(op.checkins.faltas[k]) { var a=buscarFuncionarioPorId(k); if(a) tbody.innerHTML += `<tr><td>${formatarDataParaBrasileiro(op.data)}</td><td style="color:red;">${a.nome}</td><td>AJUDANTE</td><td>FALTA</td><td>-</td></tr>`; }}); }
    });
};
...
window.gerarRelatorioGeral = function() {
    ...
    if (vPlaca) {
        ...
        htmlCabecalho = `<div style="background:#e3f2fd; padding:15px; border-radius:8px; margin-bottom:20px;"><h3>VEÍCULO: ${vPlaca}</h3><p>Fat: ${formatarValorMoeda(vStats.fat)} | KM: ${vStats.km} | Média: ${mediaKmL.toFixed(2)} Km/L | Lucro: ${formatarValorMoeda(vStats.lucro)}</p></div>`;
    }
    ...
    ops.forEach(op => {
        ...
        if (!op.checkins || !op.checkins.faltaMotorista) cust += (Number(op.comissao)||0);
        if(op.ajudantes) op.ajudantes.forEach(aj => { if(!(op.checkins?.faltas?.[aj.id])) cust += (Number(aj.diaria)||0); });
        var luc = rec - cust; totalFat += rec; totalLucro += luc;
        html += `<tr><td>${formatarDataParaBrasileiro(op.data)}</td><td>${op.veiculoPlaca}</td><td>${cli?cli.razaoSocial:'-'}</td><td>${formatarValorMoeda(rec)}</td><td>${formatarValorMoeda(luc)}</td></tr>`;
    });
    html += `<tr style="background:#f5f5f5; font-weight:bold;"><td colspan="3" style="text-align:right;">TOTAIS:</td><td>${formatarValorMoeda(totalFat)}</td><td>${formatarValorMoeda(totalLucro)}</td></tr></tbody></table>`;
...
window.gerarRelatorioCobranca = function() {
    ...
    var porCliente = {};
    ops.forEach(op => {
        var nome = buscarContratantePorCnpj(op.contratanteCNPJ)?.razaoSocial || 'DESCONHECIDO';
        if (!porCliente[nome]) porCliente[nome] = { ops: [], totalFat: 0, totalAdiant: 0 };
        porCliente[nome].ops.push(op);
        porCliente[nome].totalFat += (Number(op.faturamento)||0);
        porCliente[nome].totalAdiant += (Number(op.adiantamento)||0);
    });

    var html = `<div style="text-align:center;"><h2>RELATÓRIO DE COBRANÇA (LÍQUIDO)</h2></div>`;
    for (var cli in porCliente) {
        var liq = porCliente[cli].totalFat - porCliente[cli].totalAdiant;
        html += `<div style="margin-bottom:30px; border:1px solid #ccc; padding:15px;"><h3 style="background:#eee; padding:10px;">${cli}</h3><table class="data-table"><thead><tr><th>DATA</th><th>PLACA</th><th>VALOR</th><th>ADIANTAMENTO</th></tr></thead><tbody>`;
        porCliente[cli].ops.forEach(op => { html += `<tr><td>${formatarDataParaBrasileiro(op.data)}</td><td>${op.veiculoPlaca}</td><td>${formatarValorMoeda(op.faturamento)}</td><td style="color:red;">${op.adiantamento>0?'- '+formatarValorMoeda(op.adiantamento):'-'}</td></tr>`; });
        html += `</tbody><tfoot><tr style="background:#333; color:white;"><td colspan="3" style="text-align:right;">A RECEBER:</td><td>${formatarValorMoeda(liq)}</td></tr></tfoot></table></div>`;
    }
    document.getElementById('reportContent').innerHTML = html; document.getElementById('reportResults').style.display = 'block';
};

window.exportarRelatorioPDF = function() { var e = document.getElementById('reportContent'); if(!e || e.innerHTML.trim()==='') return alert("Gere o relatório."); html2pdf().set({margin:10, filename:'Relatorio.pdf'}).from(e).save(); };
...
window.enviarRecibo = async function(id) {
    ...
    if(r) { r.enviado = true; await salvarListaRecibos(CACHE_RECIBOS); renderizarHistoricoRecibos(); alert("Enviado!"); }
};
...
window.finalizarViagem = async function(id) {
    ...
    if(op) { 
        ...
        op.kmRodado = Number(km) - Number(op.kmInicial || 0); 
